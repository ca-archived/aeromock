...
trait HttpResponseWriter {
  ...
  def renderJsonp[A <: AnyRef](value: A, callbackName: String, status: HttpResponseStatus,
      customResponse: Option[CustomResponse] = None)(implicit context: ChannelHandlerContext): HttpResponse = {
    val responseStatus = selectResponseStatus(status, customResponse)
    val response = new DefaultFullHttpResponse(HTTP_1_1, responseStatus,
      Unpooled.copiedBuffer(s"$callbackName(${write(value)})", CharsetUtil.UTF_8))
    addDefaultHeader(response)
    addCustomResponse(response, customResponse)

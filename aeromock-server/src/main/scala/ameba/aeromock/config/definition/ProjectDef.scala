...
import ameba.aeromock.config.{ServerOptionRepository, TemplateConfig}
      ...
      case _ => {
        try {
          ...
          if (!parent.isAssignableFrom(clazz)) {
            message"validation.not.subclass${serviceClass}${parent.getName()}${"template.serviceClass"}".failureNel[TemplateClass]
          } else {
            clazz.asInstanceOf[TemplateClass].successNel
          }
        } catch {
          case e: ClassNotFoundException => {
            message"validation.not.exists.class${serviceClass}${"template.serviceClass"}".failureNel[TemplateClass]
          }
          case e: Throwable => {
            message"validation.fail.load.class${serviceClass}${"template.serviceClass"}".failureNel[TemplateClass]
          }
        }
      }
    }
...
class TagDef {
  ...
  type ETag = ameba.aeromock.config.entity.Tag
    ...
    root match {
      ...
      case _ => {
        ...
        if (!rootPath.exists) {
          ...
          ameba.aeromock.config.entity.Tag(rootPath).some.successNel
        }
      }
    }
  }
